 ...
 *
 * "Does not have to" is too soft, measured 2026-09-08: it MUST NOT.
 * `first_missing_ffi_lib` (compiler-codegen/src/link_prep.rs:60) walks `[ffi] libs`
 * and rejects the FIRST name with no file under `lib_dirs` -- and one such name
 * disqualifies every library that IS there, silently. A system library has no file
 * to find. Adding these six to the manifest for an hour tonight cost the whole set.
 * Registry №1055. */
